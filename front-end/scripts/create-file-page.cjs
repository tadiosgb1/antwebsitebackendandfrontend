/**
 * create-file-page.cjs
 * Generates Vue3 CRUD pages and injects routing + sidebar.
 * Usage:
 * node create-file-page.cjs PageName src views opened|closed Role field1:type field2:type ...
 */
...
const [pageName, srcFolder, type, role, ...fieldArgs] = args;

if (!pageName || !srcFolder || !type || !role) {
  console.error("Usage: node create-file-page.cjs PageName src views opened|closed Role field1:type ...");
  process.exit(1);
}

const capitalize = (str) => str.charAt(0).toUpperCase() + str.slice(1);

// Parse fields into {name,type}
const fields = fieldArgs.length
  ? fieldArgs.map(f => {
      const [name, fieldType] = f.split(":");
      return { name, type: fieldType || "text" };
    })
  : [{ name: "Field1", type: "text" }, { name: "Field2", type: "text" }];

const baseDir = path.join(process.cwd(), srcFolder, type, role, pageName);
      ...
      <form @submit.prevent="submitForm" class="space-y-4" enctype="multipart/form-data">
        ${fields.map(f => {
          if (f.type === "file") {
            return `
        <div>
          <label class="block mb-1 text-sm font-medium text-gray-700">${capitalize(f.name)}</label>
          <input @change="handleFile($event, '${f.name}')" type="file" class="border border-gray-300 rounded-lg px-4 py-2 text-sm w-full sm:max-w-xs" />
        </div>`;
          } else if (f.type === "checkbox") {
            return `
        <div class="flex items-center gap-2">
          <input v-model="form.${f.name}" type="checkbox" />
          <label class="text-gray-700">${capitalize(f.name)}</label>
        </div>`;
          } else if (f.type === "dropdown") {
            return `
        <div>
          <label class="block mb-1 text-sm font-medium text-gray-700">${capitalize(f.name)}</label>
          <select v-model="form.${f.name}" class="border border-gray-300 rounded-lg px-4 py-2 text-sm w-full sm:max-w-xs">
            <option value="">Select ${capitalize(f.name)}</option>
          </select>
        </div>`;
          } else {
            return `
        <div>
          <label class="block mb-1 text-sm font-medium text-gray-700">${capitalize(f.name)}</label>
          <input v-model="form.${f.name}" type="${f.type}" required class="border border-gray-300 rounded-lg px-4 py-2 text-sm w-full sm:max-w-xs focus:outline-none focus:ring-2 focus:ring-green-500 shadow-sm transition duration-150" />
        </div>`;
          }
        }).join('')}
...
export default {
  ...
  data() {
    return {
      form: {
        ${fields.map(f => `${f.name}: ${f.type === "checkbox" ? "false" : "''"}`).join(",\n")}
      },
      files: {}
    };
  },
  methods: {
    ...
    async submitForm() {
      try {
        ...
        if ("${mode}" === "Add") {
          await this.$apiPost("/${pageName.toLowerCase()}", formData, headers);
          this.$root.$refs.toast.showToast('Added successfully', 'success');
        } else {
          await this.$apiPut("/${pageName.toLowerCase()}/" + this.data.id, formData, headers);
...
<template>
  <div class="p-6 bg-gray-50 min-h-screen text-sm text-gray-800 relative">
        ...
        <table class="min-w-full text-sm divide-y divide-gray-200">
          <thead class="bg-gray-100 text-gray-700 uppercase text-xs font-semibold">
            <tr>
              ...
              ${fields.map(f => `<th class="px-6 py-3 text-left">${capitalize(f.name)}</th>`).join('')}
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="(item, index) in items" :key="item.id" class="hover:bg-green-50 transition duration-150">
              ...
              ${fields.map(f => `<td class="px-6 py-4 whitespace-nowrap">{{ item.${f.name} }}</td>`).join('')}
            ...
            <tr v-if="items.length===0"><td colspan="${fields.length + 2}" class="text-center py-6 text-gray-400 italic">No data found.</td></tr>
          </tbody>
        </table>
      </div>
    </div>
    ...
    <div class="md:hidden space-y-4">
      <div v-for="(item,index) in items" :key="item.id" class="bg-white border border-gray-200 rounded-xl shadow p-4">
        ...
        <div class="grid grid-cols-2 gap-y-1 text-sm text-gray-700">
          ${fields.map(f => `
            <div class="col-span-2"><span class="font-medium text-gray-600">${capitalize(f.name)}:</span> {{ item.${f.name} }}</div>`).join('')}
        </div>
      </div>
    ...
    <delete-confirm-modal :visible="deleteModalVisible" title="Delete ${capitalize(pageName)}" message="Are you sure you want to delete this ${pageName}?" @confirm="confirmDelete" @cancel="deleteModalVisible=false" />
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    viewDetails(id){ this.$router.push({ name:'${capitalize(pageName)}-detail', params:{id}}); },
    openDeleteModal(id){ this.deleteId=id; this.deleteModalVisible=true; },
    async confirmDelete(){ const res = await this.$apiDelete('/${pageName.toLowerCase()}', this.deleteId); if(res) this.$root.$refs.toast.showToast('${capitalize(pageName)} deleted successfully', 'success'); this.deleteModalVisible=false; this.fetchItems(this.currentPage);}
  },
...
<template>
  <div class="p-6 bg-gray-50 min-h-screen text-sm text-gray-800">
    ...
    <div class="bg-white overflow-hidden rounded-md border border-gray-200 p-4 hidden md:block space-y-2">
      ...
      ${fields.map(f=>`<div><strong>${capitalize(f.name)}:</strong> {{ item.${f.name} }}</div>`).join('')}
    </div>
...
export default {
  ...
  async mounted(){
    this.loading=true;
    try{ this.item = await this.$apiGetById('/${pageName.toLowerCase()}', this.$route.params.id) || {}; }
    catch(e){console.error(e);} finally {this.loading=false;}
  }
}
</script>
  ...
  const routeSnippet = `
    {
      ...
      component: () => import('../${type}/${role}/${pageName}/${capitalize(pageName)}View.vue'),
    },
    {
      path: "${pageName.toLowerCase()}/add",
      name: "${capitalize(pageName)}-add",
      component: () => import('../${type}/${role}/${pageName}/Add${capitalize(pageName)}.vue'),
    },
    {
      path: "${pageName.toLowerCase()}/edit/:id",
      name: "${capitalize(pageName)}-edit",
      component: () => import('../${type}/${role}/${pageName}/Edit${capitalize(pageName)}.vue'),
      props:true,
    },
    {
      path: "${pageName.toLowerCase()}/detail/:id",
      name: "${capitalize(pageName)}-detail",
      component: () => import('../${type}/${role}/${pageName}/${capitalize(pageName)}Detail.vue'),
      props:true,
    },`;
...
} else { console.warn("⚠ Router file not found. Skipping route injection."); }
...
if (fs.existsSync(sidebarPath)) {
  ...
  const sidebarSnippet = `
    { name: "${capitalize(pageName)}", route:"${capitalize(pageName)}-view", icon:"fas fa-folder", color:"#22c55e" },`;
  sidebarContent = sidebarContent.replace(/(menuItems:\s*\[)/, `$1\n${sidebarSnippet}`);
  fs.writeFileSync(sidebarPath, sidebarContent);
  console.log(`✅ Sidebar updated with ${capitalize(pageName)}.`);
} else { console.warn("⚠ Sidebar not found. Skipping menu update."); }
