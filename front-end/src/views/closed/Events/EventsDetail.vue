...
<template>
  <div class="p-6 bg-gray-50 min-h-screen text-sm text-gray-800">
    ...
    <div class="bg-white overflow-hidden rounded-md border border-gray-200 p-4 hidden md:block space-y-2">
      ...
      <div><strong>Events:</strong> {{ item.Events }}</div><div><strong>Title:</strong> {{ item.title }}</div><div><strong>Description:</strong> {{ item.description }}</div><div><strong>Image:</strong> {{ item.image }}</div><div><strong>Video:</strong> {{ item.video }}</div><div><strong>Start_date:</strong> {{ item.start_date }}</div><div><strong>End_date:</strong> {{ item.end_date }}</div><div><strong>Category_id:</strong> {{ item.category_id }}</div>
    </div>
...
export default {
  ...
  async mounted(){
    this.loading=true;
    try{ this.item = await this.$apiGetById('/event', this.$route.params.id) || {}; }
