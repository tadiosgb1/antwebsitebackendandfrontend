<template>
  <div class="p-6 bg-gray-50 min-h-screen text-sm text-gray-800 relative">
    <!-- Loading -->
    <Loading :visible="loading" message="Loading Gallery..." />

    <!-- Header -->
    <div class="flex items-center justify-between mb-6 border-b pb-4 border-gray-200">
      <h1 class="text-lg font-bold text-gray-800">Gallery</h1>
    ...
    <!-- Modals -->
    <add-gallery v-if="showModal && !editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>
    <edit-gallery v-if="showModal && editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>
    <delete-confirm-modal :visible="deleteModalVisible" title="Delete Gallery" message="Are you sure you want to delete this Gallery?" @confirm="confirmDelete" @cancel="deleteModalVisible=false" />
...
export default {
  components: { AddGallery, EditGallery, Loading, DeleteConfirmModal },
  data() {
    return {
      items: [],
      count: 0,
  ...
  methods: {
    async fetchItems(page = 1) {
      ...
      try {
        const response = await this.$apiGet('/gallery', params);
    ...
    async confirmDelete() {
      const res = await this.$apiDelete('/gallery', this.deleteId);
      if (res) this.$root.$refs.toast.showToast('Gallery deleted successfully', 'success');
      this.deleteModalVisible = false;
      this.fetchItems(this.currentPage);
    },
    viewDetails(id) {
      this.$router.push({ name: 'Gallery-detail', params: { id } });
    }
  },
  mounted() {
    this.fetchItems();
