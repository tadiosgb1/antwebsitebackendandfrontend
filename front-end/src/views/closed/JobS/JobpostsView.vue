...
<template>
  <div class="p-6 bg-gray-50 min-h-screen text-sm text-gray-800 relative">
    ...
    <div class="flex items-center justify-between mb-6 border-b pb-4 border-gray-200">
      <h1 class="text-lg font-bold text-gray-800">Jobposts</h1>
        ...
        <table class="min-w-full text-sm divide-y divide-gray-200">
          <thead class="bg-gray-100 text-gray-700 uppercase text-xs font-semibold">
            <tr>
              ...
              <th class="px-6 py-3 text-left">Description</th><th class="px-6 py-3 text-left">
                Location</th>
                <th class="px-6 py-3 text-left">
                  Employment_type</th>
                  <th class="px-6 py-3 text-left">
                    Experience_level</th>
                    <th class="px-6 py-3 text-left">Salary_min</th>
                    <th class="px-6 py-3 text-left">Salary_max</th>
                    <th class="px-6 py-3 text-left">Vacancies</th>
                    <th class="px-6 py-3 text-left">Deadline</th>
                    <th class="px-6 py-3 text-left">Is_active</th>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="(item, index) in items" :key="item.id" class="hover:bg-green-50 transition duration-150">
              ...
              <td class="px-6 py-4 whitespace-nowrap">{{ item.title }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.description }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.location }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.employment_type }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.experience_level }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.salary_min }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.salary_max }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.vacancies }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.deadline }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.is_active }}</td>
    ...
    <div class="md:hidden space-y-4">
      <div v-for="(item, index) in items" :key="item.id" class="bg-white border border-gray-200 rounded-xl shadow p-4">
        <div class="flex justify-between mb-3">
          <h2 class="font-bold text-gray-800">Jobposts #{{ index + 1 }}</h2>
        ...
        <div class="grid grid-cols-2 gap-y-1 text-sm text-gray-700">
            ...
            <div class="col-span-2">
              <span class="font-medium text-gray-600">Employment_type:</span>
              {{ item.employment_type }}
            </div>
            <div class="col-span-2">
              <span class="font-medium text-gray-600">Expereince_level:</span>
              {{ item.experience_level }}
            </div>
            <div class="col-span-2">
              <span class="font-medium text-gray-600">Salary_min:</span>
              {{ item.salary_min }}
            </div>
            <div class="col-span-2">
              <span class="font-medium text-gray-600">Salary_max:</span>
              {{ item.salary_max }}
            </div>
            <div class="col-span-2">
              <span class="font-medium text-gray-600">Vacancies:</span>
              {{ item.vacancies }}
            </div>
            <div class="col-span-2">
    ...
    <!-- Add/Edit Modal -->
    <add-jobposts v-if="showModal && !editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>
    <edit-jobposts v-if="showModal && editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>

    <!-- Delete Confirmation Modal -->
    <delete-confirm-modal 
      :visible="deleteModalVisible"
      title="Delete Jobposts"
      message="Are you sure you want to delete this Jobposts?"
...
<script>
import AddJobposts from "./AddJobposts.vue";
import EditJobposts from "./EditJobposts.vue";
import Loading from "@/components/Loading.vue";
import DeleteConfirmModal from "@/components/DeleteConfirmModal.vue";

export default {
  components: { AddJobposts, EditJobposts, Loading, DeleteConfirmModal },
  ...
  methods: {
    async fetchItems(page = 1) {
      ...
      try {
        const response = await this.$apiGet('/jobposts', params);
    ...
    viewDetails(id) { 
      this.$router.push({ name: 'Jobposts-detail', params: { id } });
    },
    ...
    // Delete with toast
    async confirmDelete() {
      const res = await this.$apiDelete('/jobposts', this.deleteId);
      if(res) {
        this.$root.$refs.toast.showToast('Jobposts deleted successfully', 'success');
      }
