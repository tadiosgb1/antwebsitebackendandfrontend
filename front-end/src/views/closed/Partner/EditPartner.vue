  ...
  <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-sm p-6 text-sm">
      <div class="flex justify-between items-center mb-4 border-b pb-2">
        <h2 class="text-lg font-semibold text-gray-800">Edit Partner</h2>
      ...
      <form @submit.prevent="submitForm" class="space-y-4" enctype="multipart/form-data">
        ...
        <div>
          ...
          <input @change="handleFile($event, 'logo')" type="file" class="border border-gray-300 rounded-lg px-4 py-2 text-sm w-full sm:max-w-xs" />
        </div>
        <div>
          <label class="block mb-1 text-sm font-medium text-gray-700">Website</label>
          <input v-model="form.website" type="text" required class="border border-gray-300 rounded-lg px-4 py-2 text-sm w-full sm:max-w-xs focus:outline-none focus:ring-2 focus:ring-green-500 shadow-sm transition duration-150" />
        </div>
  ...
  methods: {
    ...
    async submitForm() {
      try {
        ...
        if ("Edit" === "Add") {
          await this.$apiPost("/partner", formData, headers);
          this.$root.$refs.toast.showToast('Added successfully', 'success');
        } else {
          await this.$apiPut("/partner" ,this.data.id, formData, headers);
