...
<template>
  <div class="p-6 bg-gray-50 min-h-screen text-sm text-gray-800">
    ...
    <div class="bg-white overflow-hidden rounded-md border border-gray-200 p-4 hidden md:block space-y-2">
      ...
      <div><strong>Name:</strong> {{ item.name }}</div><div><strong>Email:</strong> {{ item.email }}</div><div><strong>Phone:</strong> {{ item.phone }}</div><div><strong>Message:</strong> {{ item.message }}</div>
    </div>
...
export default {
  ...
  async mounted() {
    ...
    try {
      const response = await this.$apiGetById('/contact', id);
      this.item = response || {};
    } catch (error) {
      console.error(error);
    } finally {
