...
module.exports = {
  async getAll(req, res) {
    try {
      ...
      const { rows, count } = await Faqs.findAndCountAll(findOptions);
  ...
  async getOne(req, res) {
    try {
      ...
      const data = await Faqs.findByPk(req.params.id, opts);
  ...
  async create(req, res) {
    try {
      ...
      const data = await Faqs.create(body);
      res.json(data);
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  ...
  async update(req, res) {
    try {
      ...
      await Faqs.update(body, { where: { id: req.params.id } });
      const updated = await Faqs.findByPk(req.params.id);
      res.json(updated);
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  ...
  async delete(req, res) {
    try {
      await Faqs.destroy({ where: { id: req.params.id } });
