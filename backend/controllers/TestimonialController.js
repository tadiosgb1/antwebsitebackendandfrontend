...
module.exports = {
  async getAll(req, res) {
    try {
      ...
      const searchableFields = ["name", "position", "content"];
      ...
      const { rows, count } = await Testimonial.findAndCountAll(findOptions);

      const host = `${req.protocol}://${req.get("host")}`;
      const dataWithFiles = rows.map(item => {
        const obj = item.toJSON();
        if (obj.avatar) obj.avatar = host + "/" + obj.avatar.replace(/\\/g, "/");
        return obj;
      });
  ...
  async getOne(req, res) {
    try {
      ...
      const data = await Testimonial.findByPk(req.params.id, opts);
  ...
  async create(req, res) {
    try {
      const body = { ...req.body };
      if (req.files && req.files['avatar']) body.avatar = req.files['avatar'][0].path;
  ...
  async update(req, res) {
    try {
      const body = { ...req.body };
      if (req.files && req.files['avatar']) body.avatar = req.files['avatar'][0].path;
      await Testimonial.update(body, { where: { id: req.params.id } });
