...
module.exports = {
  async getAll(req, res) {
    try {
      ...
      const include = [db.Category, db.User].filter(Boolean);
      const findOptions = { where, order, offset, limit: page_size };
      if (include.length) findOptions.include = include;

      const { rows, count } = await News1.findAndCountAll(findOptions);
  ...
  async getOne(req, res) {
    try {
      ...
      const data = await News1.findByPk(req.params.id, opts);
  ...
  async create(req, res) {
    try {
      ...
      const data = await News1.create(body);
      res.json(data);
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  ...
  async update(req, res) {
    try {
      ...
      if (req.files && req.files['image']) body.image = req.files['image'][0].path;
      if (req.files && req.files['video']) body.video = req.files['video'][0].path;
      await News1.update(body, { where: { id: req.params.id } });
      const updated = await News1.findByPk(req.params.id);
      res.json(updated);
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  ...
  async delete(req, res) {
    try {
      await News1.destroy({ where: { id: req.params.id } });
