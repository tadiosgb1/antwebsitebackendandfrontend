...
module.exports = {
  async getAll(req, res) {
    try {
      ...
      const searchableFields = ["title", "slug", "summary", "technologies", "industry", "demo_url"];
      ...
      const { rows, count } = await Solutions.findAndCountAll(findOptions);

      const host = `${req.protocol}://${req.get("host")}`;
      const dataWithFiles = rows.map(item => {
        const obj = item.toJSON();
        if (obj.thumbnail) obj.thumbnail = host + "/" + obj.thumbnail.replace(/\\/g, "/");
        return obj;
      });
  ...
  async getOne(req, res) {
    try {
      ...
      const data = await Solutions.findByPk(req.params.id, opts);
  ...
  async create(req, res) {
    try {
      const body = { ...req.body };
      if (req.files && req.files['thumbnail']) body.thumbnail = req.files['thumbnail'][0].path;
      const data = await Solutions.create(body);
      res.json(data);
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  ...
  async update(req, res) {
    try {
      const body = { ...req.body };
      if (req.files && req.files['thumbnail']) body.thumbnail = req.files['thumbnail'][0].path;
      await Solutions.update(body, { where: { id: req.params.id } });
      const updated = await Solutions.findByPk(req.params.id);
      res.json(updated);
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  ...
  async delete(req, res) {
    try {
      await Solutions.destroy({ where: { id: req.params.id } });
