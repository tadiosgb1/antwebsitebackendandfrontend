...
module.exports = {
  async getAll(req, res) {
    try {
      ...
      const searchableFields = ["name", "role", "facebook", "linkedin", "instagram", "twitter", "telegram"];
      ...
      const { rows, count } = await Team.findAndCountAll(findOptions);

      const host = `${req.protocol}://${req.get("host")}`;
      const dataWithFiles = rows.map(item => {
        const obj = item.toJSON();
        if (obj.photo) obj.photo = host + "/" + obj.photo.replace(/\\/g, "/");
        return obj;
      });
  ...
  async getOne(req, res) {
    try {
      ...
      const data = await Team.findByPk(req.params.id, opts);
  ...
  async create(req, res) {
    try {
      const body = { ...req.body };
      if (req.files && req.files['photo']) body.photo = req.files['photo'][0].path;
  ...
  async update(req, res) {
    try {
      const body = { ...req.body };
      if (req.files && req.files['photo']) body.photo = req.files['photo'][0].path;
      await Team.update(body, { where: { id: req.params.id } });
      const updated = await Team.findByPk(req.params.id);
      res.json(updated);
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
