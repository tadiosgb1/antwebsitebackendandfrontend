...
module.exports = {
  async getAll(req, res) {
    try {
      ...
      const { rows, count } = await Gallery.findAndCountAll(findOptions);
  ...
  async getOne(req, res) {
    try {
      ...
      const data = await Gallery.findByPk(req.params.id, opts);
  ...
  async update(req, res) {
    try {
      const body = { ...req.body };
      if (req.files && req.files['image']) body.image = req.files['image'][0].path;
      await Gallery.update(body, { where: { id: req.params.id } });
