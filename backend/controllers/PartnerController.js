...
module.exports = {
  async getAll(req, res) {
    try {
      ...
      const searchableFields = ["name", "website"];
      ...
      const { rows, count } = await Partner.findAndCountAll(findOptions);
  ...
  async getOne(req, res) {
    try {
      ...
      const data = await Partner.findByPk(req.params.id, opts);
  ...
  async update(req, res) {
    try {
      const body = { ...req.body };
      if (req.files && req.files['logo']) body.logo = req.files['logo'][0].path;
      await Partner.update(body, { where: { id: req.params.id } });
      const updated = await Partner.findByPk(req.params.id);
      res.json(updated);
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
