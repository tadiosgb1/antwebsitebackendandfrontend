...
module.exports = {
  async getAll(req, res) {
    try {
      ...
      const searchableFields = ["title", "location", "employment_type", "experience_level"];
      ...
      const { rows, count } = await JobPosts.findAndCountAll(findOptions);
  ...
  async getOne(req, res) {
    try {
      ...
      const data = await JobPosts.findByPk(req.params.id, opts);
  ...
  async create(req, res) {
    try {
      ...
      const data = await JobPosts.create(body);
      res.json(data);
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  ...
  async update(req, res) {
    try {
      ...
      await JobPosts.update(body, { where: { id: req.params.id } });
      const updated = await JobPosts.findByPk(req.params.id);
      res.json(updated);
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  ...
  async delete(req, res) {
    try {
      await JobPosts.destroy({ where: { id: req.params.id } });
